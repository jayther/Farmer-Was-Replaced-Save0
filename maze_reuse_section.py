...
def create_run(col_start, col_end, max_gold = -1, timer_enabled_arg = False):
	...
	def find_treasure():
		...
		while len(path) > 0:
			...
			for dir in dirs:
				...
				if dir_coords[0] < 0 or dir_coords[0] >= maze_size:
					continue
				if dir_coords[1] < 0 or dir_coords[1] >= maze_size:
					continue
